            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className="fixed inset-0 z-40 bg-navy/95 backdrop-blur-xl flex flex-col justify-center items-center gap-8 p-6 md:hidden"
                    >
                        {navLinks.map((link) => (
                            <Link
                                key={link.name}
                        ...
                        <div className="flex flex-col gap-4 w-full max-w-xs mt-8">
                            <Link href="/sell" onClick={() => setIsMobileMenuOpen(false)}>
                                <Button variant="outline" className="w-full">Seller Access</Button>
                            </Link>
                            <Link href="/buy" onClick={() => setIsMobileMenuOpen(false)}>
                                <Button variant="primary" className="w-full">Buyer Access</Button>
                            </Link>
                        </div>
                    </motion.div>
                )}
